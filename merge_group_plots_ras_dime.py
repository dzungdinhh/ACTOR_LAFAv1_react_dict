#!/usr/bin/env python3
"""
Create merged 3x2 ADNI group panels from learned / RAS / DIME rollout data.

Rows: RAS / DIME / learned
...
def make_dark_group_colors(num_groups: int):
    colors = []
...
def _render_learned_context_plot(group_dir: str, out_file_path: str, dataset: str):
    """
    Copy the learned-context figure from the source matched group.
    """
    ...
    candidate_paths = []
    learned_ctx = meta.get("learned_baseline_context", {})
    if isinstance(learned_ctx, dict):
        candidate_paths.append(str(learned_ctx.get("plot", "")))
    source_group_dir = str(meta.get("source_group_dir", ""))
    if source_group_dir:
        candidate_paths.append(os.path.join(source_group_dir, "learned_baseline_context_binary.png"))

    for src_plot in candidate_paths:
        if src_plot and os.path.isfile(src_plot):
            out_parent = os.path.dirname(out_file_path)
            os.makedirs(out_parent, exist_ok=True)
            shutil.copy2(src_plot, out_file_path)
            return out_file_path
    raise FileNotFoundError(f"Missing learned context plot for group: {group_dir}")
...
def _centroids_from_masks(
    ...
    raw = centers.reshape(k, num_time, num_groups)[:, :, keep_idx]
...
def _plot_heatmap_cell(
    ...
    for i in range(k):
        ...
        if i == 0:
            ...
            ax.set_yticklabels([short_text(ordered_names[j], max_len=24) for j in yticks], fontsize=7)
        else:
            ax.set_yticks(yticks)
            ax.set_yticklabels([])
...
def _plot_trajectory_cell(
    ...
    # Row 3: policy graph
    colors = make_dark_group_colors(f)
    for y in range(f):
        for tt in range(t):
    ...
    if int(label_stride) > 1:
        ...
        ax_graph.set_yticklabels([short_text(ordered_names[i], 24) for i in yt], fontsize=font_size, fontweight="bold")
    else:
        ax_graph.set_yticklabels([short_text(x, 24) for x in ordered_names], fontsize=font_size, fontweight="bold")
...
def _render_group(group_dir: str, out_file_path: str, args):
    ...
    auroc_triplet = _fmt_triplet(costs, "auroc")
    auprc_triplet = _fmt_triplet(costs, "auprc")
    fig.suptitle(
        (
            f"{str(args.dataset).lower()} | total cost: {total_triplet} | longitudinal cost: {acq_triplet}\n"
            f"AUROC: {auroc_triplet} | AUPRC: {auprc_triplet}"
        ),
        fontsize=15,
        y=0.996,
    )
    plt.subplots_adjust(left=float(left_margin), right=0.995, top=0.925, bottom=0.03)
